...
const travellingBlogs = ({posts}) => {
  return (
    ...
    <header className="text-center py-10 bg-gradient-to-r from-blue-500 to-indigo-600 text-white">
        ...
        <h2 className="text-3xl md:text-5xl font-thin">Travelling Blog</h2>
      </header>
...
export async function getServerSideProps(context) {
    ...
    const posts = await blog.find({ category: 'travelling' });
    return {
      props: { posts: posts ? JSON.parse(JSON.stringify(posts)) : [] },
    };
  }

export default travellingBlogs