        ...
        <div className="grid md:grid-cols-2 items-center gap-4 max-w-6xl w-full">
          <div className="border border-gray-300 rounded-lg p-6 max-w-md shadow-[0_2px_22px_-4px_rgba(93,96,127,0.2)] max-md:mx-auto">
            <form onSubmit={handleSubmit} method="POST" className="space-y-4">
              <div className="mb-8">
                <h3 className="text-gray-800 dark:text-white text-3xl font-extrabold">Sign in</h3>
                <p className="text-gray-500 dark:text-white/60 text-sm mt-4 leading-relaxed">Sign in to your account and explore a world of possibilities. Your journey begins here.</p>
              </div>

              <div>
                <label className="text-gray-800 dark:text-white/80 text-sm mb-2 block">User email</label>
                <div className="relative flex items-center">
                  <input name="email"
                  value={email}
                  onChange={handleChange}
                  type="text" required className="w-full text-sm text-gray-800 border border-gray-300 px-4 py-3 rounded-lg outline-blue-600" placeholder="Enter user name" />
              ...
              <p className="text-sm !mt-8 text-center dark:text-white/80 text-gray-800">Don't have an account <Link href="/singup" className="text-blue-600 font-semibold hover:underline ml-1 whitespace-nowrap">Register here</Link></p>
            </form>
          </div>
